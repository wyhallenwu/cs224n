...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors,
    ...
    ### to integer overflow.
    result_softmax = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(result_softmax[outsideWordIdx])

    result_softmax[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, result_softmax)
    gradOutsideVecs = np.dot(np.expand_dims(result_softmax, axis=1),
...
def negSamplingLossAndGradient(centerWordVec,
    ...
    in_dataset_pair = outsideVectors[outsideWordIdx].T.dot(centerWordVec)
    negativesamples = outsideVectors[negSampleWordIndices]
    out_dataset_pair = -negativesamples.dot(centerWordVec)
    loss = -np.log(sigmoid(in_dataset_pair)) - np.sum(
        np.log(sigmoid(out_dataset_pair)))

    gradCenterVec = -(1 - sigmoid(in_dataset_pair)) * outsideVectors[
        outsideWordIdx] + np.sum(
            (np.expand_dims(1 - sigmoid(out_dataset_pair), axis=1)) *
            negativesamples,
            axis=0)

    gradOutsideVecs = np.zeros(shape=outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -(
        1 - sigmoid(in_dataset_pair)) * centerWordVec

    for i, negi in enumerate(negSampleWordIndices):
        gradOutsideVecs[negi] += (1 -
                                  sigmoid(out_dataset_pair)[i]) * centerWordVec
    ...
    for word in outsideWords:
        loss_gradient = list(
            word2vecLossAndGradient(
                centerWordVectors[word2Ind[currentCenterWord]], word2Ind[word],
                outsideVectors, dataset))

        loss += loss_gradient[0]
        gradCenterVecs[word2Ind[currentCenterWord]] += loss_gradient[1]
        gradOutsideVectors += loss_gradient[2]
...
def test_sigmoid():
    ...
    assert np.allclose(sigmoid(np.array([1, 2, 3])),
...
def test_naiveSoftmaxLossAndGradient():
    ...
    gradcheck_naive(temp, np.random.randn(3),
                    "naiveSoftmaxLossAndGradient gradCenterVec")
...
def test_negSamplingLossAndGradient():
    ...
    gradcheck_naive(temp, np.random.randn(3),
                    "negSamplingLossAndGradient gradCenterVec")
